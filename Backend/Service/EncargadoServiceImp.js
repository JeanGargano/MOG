...
export class EncargadoService {
  ...
  async findByIdentificacion(identificacion) {
    ...
    const encargado = await this.encargadoRepository.findByIdentificacion(identificacion);
    if (!encargado) {
  ...
  async postEncargado(data) {
    ...
    const encargadoExistente = await this.encargadoRepository.findByIdentificacion(data.identificacion);
  ...
  async agregarCampos(identificacion, nuevosCampos) {
    ...
    if (plainPassword) {
      if (typeof plainPassword !== 'string' || plainPassword.length <= 4) {
        throw new Error("La nueva contraseña debe ser una cadena válida de al menos 4 caracteres");
      }
      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash(plainPassword, salt);
      nuevosCampos.contraseña = hashedPassword;
    }
    const actualizado = await this.encargadoRepository.agregarCampos(identificacion, nuevosCampos);
    if (!actualizado) {
  ...
  async logearEncargado({ identificacion, contraseña }) {
    ...
    const validPassword = await bcrypt.compare(contraseña, encargado.contraseña);
    if (!validPassword) {
